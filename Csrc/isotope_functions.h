...
double microphysics_g_iso(struct LookupStruct *LT, double (*lam_fp)(double), double (*L_fp)(double, double), double temperature, 
                          double D_vapor, double D_O18, double kt, double p0, double sat_ratio,
                          double qr, double qr_iso, double qv, double qv_iso){
    double lam          = lam_fp(temperature);
    double L            = L_fp(temperature,lam);
    double pv_sat       = lookup(LT, temperature);
    double rho_sat      = pv_sat/Rv/temperature;
...
void sb_iso_evaporation_rain(struct LookupStruct *LT, double (*lam_fp)(double), double (*L_fp)(double, double), double temperature, double p0, 
                             double sat_ratio, double nr, double mu, double rain_mass, double Dp, double Dm,
                             double qr, double qr_iso, double qv, double qv_iso, double* qr_iso_tendency){
    ...
    if(qr < SB_EPS || nr < SB_EPS || qr_iso < SB_EPS){
        ...
        *qr_iso_tendency = 0.0;
        ...
        double sat_ratio_iso = sat_ratio + 1.0;
        double g_therm_iso = microphysics_g_iso(LT, lam_fp, L_fp, temperature, DVAPOR, D_O18, KT, p0, sat_ratio_iso, qr, qr_iso, qv, qv_iso);
        gamma = 0.7; 
        // gamma = 0.7 is used by DALES ; alternative expression gamma= d_eq/Dm * exp(-0.2*mudouble vapor_H2O18_diff, vapor_HDO16_diff; 2double vapor_H2O18_diff, vapor_HDO16_diff;) is used by S08;
